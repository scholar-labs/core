...
export async function find(sourceCode, pattern) {
    ...
    // Wrap the pattern in an expression statement to ensure proper parsing
    const wrappedPattern = `(${pattern});`;
    const formattedPattern = await format(wrappedPattern);

    const sourceTree = parser.parse(formattedSourceCode);
    const allNodes = Array.from(traverseTree(sourceTree));
    const patternTree = parser.parse(formattedPattern);

    // Extract the actual pattern node from the wrapped expression
    let targetNode = patternTree.rootNode.firstChild; // expression_statement
    if (targetNode.firstChild.type === 'parenthesized_expression') {
